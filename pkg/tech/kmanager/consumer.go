...
func NewMessageConsumer(ctx context.Context, db *gorm.DB, cfg *config.KafkaConfig, clientID, consumerGroup string, topicNames []string) *MessageConsumer {
	return createMessageConsumer(ctx, db, cfg, clientID, consumerGroup, topicNames, false)
}

func NewMessageConsumerOrigin(ctx context.Context, db *gorm.DB, cfg *config.KafkaConfig, clientID, consumerGroup string, topicNames []string) *MessageConsumer {
	return createMessageConsumer(ctx, db, cfg, clientID, consumerGroup, topicNames, true)
}

func createMessageConsumer(ctx context.Context, db *gorm.DB, cfg *config.KafkaConfig, clientID, consumerGroup string, topicNames []string, origin bool) *MessageConsumer {
